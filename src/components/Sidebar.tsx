...
export default function Sidebar() {
    const { currentView, setView, alerts, logout, user, selectedUnit, units, mobileMenuOpen, setMobileMenuOpen } = useApp();
    ...
    return (
        <>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out flex w-[220px] flex-col flex-shrink-0 h-screen`}
                ...
                <div className="flex items-center gap-3 px-5 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    {/* Mobile Close Button */}
                    <button onClick={() => setMobileMenuOpen(false)} className="ml-auto md:hidden text-[#7A8299] hover:text-white">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="px-3 py-4 space-y-1" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                    <div className="px-3 py-1">
                        ...
                        <div className="text-[10px] font-mono text-[#7A8299]">SN: {unit?.serial ?? 'N/A'}</div>
                    </div>
                    <button
                        onClick={logout}
